<?php
// edit_documents.php - Página para editar documentos (solo accesible por administradores)
...
$sql = "SELECT d.IDDocumento, d.Codigo, d.Nombre_Documento, td.NombreTipo, l.NombreLinea, e.NombreEstacion, c.NombreCategoria, v.NumeroVersion, 
        CASE 
            WHEN ed.IDUsuario IS NOT NULL THEN CONCAT(u.Nombre, ' ', u.ApellidoPaterno, ' ', u.ApellidoMaterno)
            ELSE 'No modificado'
        END AS EditadoPor
        FROM Documentos d 
        ...
        LEFT JOIN EdicionesDocumento ed ON d.IDDocumento = ed.IDDocumento 
        LEFT JOIN Usuarios u ON ed.IDUsuario = u.IDUsuario 
        WHERE d.Activo = TRUE";
$filters = [];
$params = [];
...
<head>
    ...
    <title>Editar Documentos - Sistema de Gestión de Archivos MI TELEFÉRICO</title>
    <!-- Estilos CSS -->
    <style>
        * {
            margin: 0;
                ...
                <tbody>
                    <?php foreach ($documentos as $doc): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($doc['EditadoPor']); ?></td>
                            <td><a href="edit_document.php?id=<?php echo $doc['IDDocumento']; ?>" class="edit-btn">✏️</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
