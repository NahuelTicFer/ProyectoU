<?php
// delete_documents.php - Página para eliminar documentos (solo accesible por administradores)
...
// Consulta base para documentos con filtros del servidor
$documentos = [];
$sql = "SELECT d.IDDocumento, d.Codigo, d.Nombre_Documento, td.NombreTipo, l.NombreLinea, e.NombreEstacion, c.NombreCategoria, v.NumeroVersion 
        FROM Documentos d 
...
// Manejo de la eliminación lógica
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
    $idDocumento = (int)$_POST['delete_id'];
    $idUsuario = $_SESSION['idUsuario'];
    $fechaEliminacion = date('Y-m-d H:i:s');

    $stmt = $conn->prepare("UPDATE Documentos SET Activo = 0 WHERE IDDocumento = ?");
    $stmt->bind_param("i", $idDocumento);
    if ($stmt->execute()) {
        $stmtEliminacion = $conn->prepare("INSERT INTO EliminacionesDocumento (IDDocumento, IDUsuario, FechaEliminacion) VALUES (?, ?, ?)");
        $stmtEliminacion->bind_param("iis", $idDocumento, $idUsuario, $fechaEliminacion);
        $stmtEliminacion->execute();
        $stmtEliminacion->close();
        $success = "Documento eliminado exitosamente (borrado lógico).";
    } else {
...
<head>
    ...
    <title>Eliminar Documentos - Sistema de Gestión de Archivos MI TELEFÉRICO</title>
    <!-- Estilos CSS -->
    <style>
        * {
            margin: 0;
...
<body>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" style="display: inline;">
                                    <input type="hidden" name="delete_id" value="<?php echo $doc['IDDocumento']; ?>">
                                    <button type="submit" class="delete-btn" onclick="return confirm('¿Está seguro que desea eliminar el documento <?php echo addslashes(htmlspecialchars($doc['Nombre_Documento'])); ?>?')">🗑️</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <script>
        ...
        function updateEstaciones() {
            ...
            if (estacionSelect && '<?php echo isset($_GET['id_estacion']) ? $_GET['id_estacion'] : ''; ?>') {
